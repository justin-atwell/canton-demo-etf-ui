import type { EtfDetail, NavDataPoint, ConstituentDetail } from '../types';

// Extended detail type for the UI — adds fields not returned by the API
// constituents, navPerShare, totalAUM come from mock/NBBO data, not the ledger ETFDefinition contract
export interface EtfDetailExtended extends EtfDetail {
  navPerShare: number;
  totalAUM: number;
  constituents: ConstituentDetail[];
}

export const MOCK_ETF_DETAIL: EtfDetailExtended = {
...
export const MOCK_NAV_HISTORY: NavDataPoint[] = Array.from({ length: 60 }, (_, i) => {
